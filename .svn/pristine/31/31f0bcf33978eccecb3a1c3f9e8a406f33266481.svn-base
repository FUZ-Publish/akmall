			...
			if (totalCount > 0) {
				...
				$.each(result.items, function(num,item){
					...
					if(hkeyword.indexOf("&lt;!CS&gt;") > 0) {
						hkeyword = hkeyword.replace(/&lt;!CS&gt;/g,"<i>").replace(/&lt;!CE&gt;/g,"</i>");
					}

					hkeyword = hkeyword.replace(/&lt;!SS&gt;/g,"").replace(/&lt;!SE&gt;/g,"");
					if(item.keyword != "압소바원주점"){
						str += "<li><a href=\"#\" id=\"bg" + num + "\" onClick=\"javascript:onClickKeyword(" + num + "); return false;\" >" +hkeyword+ "</a>";
						str += "<span id=\"f" + num + "\" class=\"apply\"><button><em class=\"blind\">"+hkeyword+"</em></button></span></li>"
					}
					});
				$(arkId).html(str).show();
	...
	$.ajax({
		...
		success: function(data) {
			...
			if (totalCount > 1) {
				...
				str +="<span class=\"related_tit\">연관</span>";
				$.each(result.items, function(num,item){
					var hkeyword = item.hkeyword;
					if(hkeyword.indexOf("&lt;!CS&gt;") > 0) {
			...
			if (totalCount > 0) {
				...
				$.each(result.items, function(num,item){
					if(num < 2) {
						...
						str += "<li>";
						str += "<a href=\"" + item.linkname + "\" >"+hkeyword+"</a>";
						str += "</li>";
...
function getArkEvent(query){
	$.ajax({
		...
		success: function(data) {
			...
			$("#ark_category .eventtitle").remove();
			$("#ark_category .event").remove();
			
			totalFwCount = totalCount;
			if (totalCount > 0) {
				// 자동완성 리스트 설정
				str += "<h1 class=\"eventtitle\">기획전</h1>";
				$.each(result.items, function(num,item){
					if(num < 2) {
						var hkeyword = item.hkeyword;
						if(hkeyword.indexOf("&lt;!CS&gt;") > 0) {
						...
						str += "<li class=\"event\">";
						str += "<a href=\"#\"  onclick=\"javascript:goShopEvent('" + item.linkname + "'); return false;\" >"+hkeyword+"</a>";
						str += "</li>";
